...
const NAV_SECTIONS = [
  {
    group: 'Portfolio',
    items: [{ id: 'misc-art', label: 'Misc Art' }],
  },
  {
    group: 'Interests',
    items: [{ id: 'misc-media', label: 'Misc Media' }],
  },
];
...
export function Sidebar({
  ...
  return html`
    <div class="contents">
      <!-- MOBILE HEADER -->
      <header
        class="md:hidden fixed top-0 left-0 w-full h-14 bg-gradient-to-b from-panel to-void z-[60] border-b border-black shadow-lg pr-4 pl-4 flex justify-between items-center cursor-none"
      >
        <button
      ...
      <aside
        class=${`fixed top-14 md:top-0 left-0 h-[calc(100dvh-56px)] md:h-[100dvh] w-64 bg-ink border-r border-white/5 z-50 flex flex-col cursor-none transition-transform duration-300 md:translate-x-0 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}
      >
        ...
        <div class="p-4 lg:p-5 border-b border-black hidden md:block shrink-0 relative z-20">
          <div class="relative w-full rounded-lg surface-hardware-well p-[2.5px] isolate">
            ...
            >
              <div
                class="absolute inset-0 pointer-events-none transition-opacity duration-300 bg-primary/5 opacity-0 group-hover:opacity-100 z-0"
              />

              <!-- Standard Text Layer (Green Glow Hover State) -->
              <span
                class="relative z-10 transition-colors duration-300 text-fog group-hover:text-primary-400"
              >
                SC // 2026
              </span>
              ...
              <span
                class="absolute inset-0 flex items-center justify-center z-20 opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none text-prismatic-foil font-bold"
              >
                SC // 2026
              </span>
            </button>
        ...
        <!-- Navigation Tree -->
        <nav
          class="flex-1 px-4 py-6 space-y-6 overflow-y-auto custom-scrollbar relative z-10 select-none"
        >
          <!-- System Control Group -->
          <div>
            <div class="px-2 mb-2">
              <h3 class="text-[10px] font-mono font-bold text-muted uppercase tracking-widest">
                / Control Deck
              </h3>
            </div>
            <ul class="space-y-1">
                    ...
                    (item) => html`
                      <li key=${item.id}>
                        ...
                        >
                          <${ChevronRight}
                            size=${12}
                            class=${activeSection === item.id ? 'text-primary' : 'text-muted'}
                          />
